...
def simulate():
    ...
    costo_almacenamiento_por_unidad_por_semana = data[
        "costo_almacenamiento_por_unidad_por_semana"
    ]
    ...
    next_inventory_check = (
        0  # Initialize the day from which inventory can be re-evaluated after an order
    )
    costo_acumulado = 0
    ...
    for dia in range(num_dias):
        ...
        entregas_para_hoy = [
            cantidad
            for entrega_dia, cantidad in entregas_pendientes
            if entrega_dia == dia
        ]
        if entregas_para_hoy:
            inventario += sum(entregas_para_hoy)
        entregas_pendientes = [
            (entrega_dia, cantidad)
            for entrega_dia, cantidad in entregas_pendientes
            if entrega_dia > dia
        ]
        ...
        if inventario < demanda_diaria:
            f_des = True
            acum_des += (
                demanda_diaria - inventario
            ) * costo_desabastecimiento_por_unidad
            inventario = 0
        else:
        ...
        costo_almacenamiento = (
            (costo_almacenamiento_por_unidad_por_semana * inventario)
            if dia % dias_por_semana == 0
            else 0
        )

        # Order placement logic
        pedido_realizado = False
        if (politica == "A" and nombre_dia == "Lunes") or (
            politica == "B" and inventario <= 5 and dia >= next_inventory_check
        ):
        ...
        costos_totales = (
            costo_almacenamiento
            + (costo_pedido if pedido_realizado else 0)
            + costo_desabastecimiento
        )
        ...
        tabla.append(
            {
                ...
                "costos_totales": costo_almacenamiento
                + (costo_pedido if pedido_realizado else 0)
                + costo_desabastecimiento,
    ...
    resultado = {
        ...
        "costo_promedio_total": sum(item["costos_totales"] for item in tabla)
        / num_dias,
        "tabla": tabla,
    }
